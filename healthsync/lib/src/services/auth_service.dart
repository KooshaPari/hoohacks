...
class AuthService {
  ...
  Future<void> initialize() async {
    try {
      ...
      final String? credentialsJson = await _storage.read(key: 'auth0_credentials');
      ...
      if (credentialsJson != null) {
        ...
        await _getUserFromDb(credentialsMap['sub'] ?? 
                            (credentialsMap['user'] != null ? 
                             credentialsMap['user']['sub'] : null));
      }
    } catch (e) {
      print('Error initializing AuthService: $e');
      // Clear possibly corrupted credentials
      await logout();
    }
  }
  ...
  Future<User?> login(String connection) async {
    try {
      if (kIsWeb && auth0Web != null) {
        ...
        // Native login
        final credentials = await auth0.webAuthentication(scheme: scheme)
  ...
  Future<User?> updateHealthConsent({bool? hasHealthkitConsent, bool? hasGoogleFitConsent}) async {
    ...
    try {
      _currentUser = await _userService.updateUser(
        ...
        _currentUser!.copyWith(
          hasHealthkitConsent: hasHealthkitConsent ?? _currentUser!.hasHealthkitConsent,
          hasGoogleFitConsent: hasGoogleFitConsent ?? _currentUser!.hasGoogleFitConsent,
          updatedAt: DateTime.now(),
        )
      );
      return _currentUser;
